...
def get_args(location: str):
    ...
    try:
        ...
        console_printer.print_error_text(f"failed to load configuration file {location} due to {ex}")
        sys.exit(-1)

def get_filenames(filenames_prefix, files_per_date, output_folder):
    r = range(files_per_date)
    t1 = [filenames_prefix+"-"+str(datetime.date.today())+"-"+f"part{i}.csv" for i in r]
    t0 = [filenames_prefix+"-"+str(datetime.date.today()-timedelta(1))+"-"+f"part{j}.csv" for j in r]
    res = t1+t0
...
def download_file(bucket, prefix, filename, local_folder):
    boto3.setup_default_session(profile_name='dataworks-development', region_name="eu-west-2")
    s3 = boto3.client('s3')
    s3.download_file(bucket, os.path.join(prefix, filename), os.path.join(local_folder, filename))

def upload_onto_s3(context, filename, prefix):
...
def dynamo_table(context):
    ...
    dynamodb = boto3.resource("dynamodb", region_name=context.args_ch['args']['region'])
...
def filename_latest_dynamo_add(context):
    ...
    myitem = {
            context.args_ch['audit-table']['hash_key']: context.args_ch['audit-table']['hash_id'],
            context.args_ch['audit-table']['range_key']: context.args_ch['audit-table']['data_product_name'],
...
def file_latest_dynamo_fetch(context):
    ...
    try:
        table = dynamo_table(context)
        response = table.query(KeyConditionExpression=Key(context.args_ch['audit-table']['hash_key']).eq(context.args_ch['audit-table']['hash_id']), ScanIndexForward=False)
        ...
        console_printer.print_error_text(f"failed to fetch last filename imported due to {ex}")
        sys.exit(-1)
