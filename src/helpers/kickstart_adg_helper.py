    ...
    try:
        ...
        if schema_config["record_layout"].lower() == "csv":
            for collection, collection_schema in schema_config["schema"].items():
                ...
                output_file_name = schema_config["output_file_pattern"].replace("run-date", run_date) \
                ...
                with open(output_file, "w+") as writer:
                    header_record = f"{schema_config['record_delimiter']}".join(collection_schema.keys())
                    writer.write(header_record+"\n")
                    num = 1
                    while num <= int(record_count):
                        record_data = f"{schema_config['record_delimiter']}".join([str(dataTypeMapping(type)()) for key, type in collection_schema.items()])
                        writer.write(record_data+"\n")
                        num += 1

        return [os.path.join(local_output_folder, x) for x in os.listdir(local_output_folder)]
...
def generate_hive_queries(schema_config, published_bucket, s3_path):
    try:
        ...
        for collection, collections_schema in schema_config["schema"].items():
            ...
            column_name = ",".join([re.sub("[^0-9a-zA-Z$]+", " ", col).strip().replace(" ", "_").lower() for col in collections_schema.keys()])
