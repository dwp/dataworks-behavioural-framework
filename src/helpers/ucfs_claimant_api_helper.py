...
def query_for_claimant_from_claimant_api(
    ...
    return send_request_to_claimant_api(
        aws_host_name, award_details_api_path, request_body, claimant_api_region
    )


def send_request_to_claimant_api(
    aws_host_name, api_path, request_body, claimant_api_region
):
    ...
    response = requests.post(
        request_url, data=json.dumps(request_body), auth=auth, headers=headers
    )
...
def hash_nino(nino, salt):
    ...
    digest_b64 = (
        base64.b64encode(sha.digest()).decode().replace("+", "-").replace("/", "_")
    )
    return digest_b64


def retrieve_assessment_periods_from_claimant_data_file(
    input_data_file_name, fixture_files_root, fixture_data_folder
):
