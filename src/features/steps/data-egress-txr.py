...
def step_submit_files_to_sft(context, data_directory):
    ...
    console_printer.print_info("Executing the following commands:'{}'".format('\n'.join(commands)))
        ...
        for dest in destinations:
            ...
            try:
                ...
                if error_code == "NoSuchKey":
                    if dest[0] != "!":
                        console_printer.print_error_text(f"Expecting collection '{collection['file']}' in destination '{dest}' on S3: {e}")
                        missing_collections_in_destination += 1
                else:
                    raise e
            except Exception as e:
                console_printer.print_error_text(f"Could not read object in S3 for collection '{collection['file']}' in destination '{dest}' on S3: {e}")
