...
def step_impl(context, record_count, module_name, PII_Flag):
    ...
    if PII_Flag.lower() == "false":
        ...
        kickstart_adg_helper.files_upload_to_s3(context, list_of_local_files, folder_name=S3_KEY_KICSKTART_TEST, upload_method="unencrypted")
        ...
        kickstart_adg_helper.files_upload_to_s3(context, list_of_local_files, folder_name=S3_KEY_KICSKTART_TEST, upload_method="encrypted")
...
def step_impl(context, modules):
    ...
    for module_name in KICKSTART_MODULES:
        ...
        additional_step_args.update({f"submit-job-{module_name}" : ["--module_name", f"{module_name}", "--e2e_test_flg", "True"]})
    ...
    for step_name in additional_step_args:
        ...
        console_printer.print_info(
            f"Step id for '{step_name}' : '{step_id}'"
        )
