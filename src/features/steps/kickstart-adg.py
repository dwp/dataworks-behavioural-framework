...
def step_impl(context, template_name, record_count, module_name, PII_Flag):
    ...
    context.kickstart_schema_config = kickstart_adg_helper.get_schema_config(context.fixture_path_local, template_name)[module_name]
    ...
    list_of_local_files = kickstart_adg_helper.generate_data(module_name,
                                                             record_count,
...
@then(
    "Start kickstart adg emr process for module '{module_name}' and wait for step '{step_name}' to complete"
)
def step_impl(context, module_name, step_name):
    ...
    emr_launcher_config = {
        "additional_step_args": {
            "submit-job": ["--correlation_id", correlation_id,
                           "--job_name", "kickstart",
    ...
    cluster_response = invoke_lambda.invoke_kickstart_adg_emr_launcher_lambda(payload_json)
...
@then(
    "Add steps '{step_name}' to kickstart adg emr cluster for '{module_name}' and wait for these steps to completed"
)
def step_impl(context, step_name, module_name):
    ...
    console_printer.print_info(
        f"generating the list of hive queries to be executed"
    )
    ...
    for hive_query in hive_queries_list:
        ...
        execution_state = aws_helper.poll_emr_cluster_step_status(
            context.pdm_cluster_step_id, context.kickstart_adg_cluster_id, 1200
        )
        if execution_state != COMPLETED_STATUS:
            raise AssertionError(
...
def step_(context, module_name):
    for collection in context.kickstart_schema_config["schema"].keys():
        s3_result_key=os.path.join(context.kickstart_hive_result_path, f"e2e_{collection}.csv")
        ...
        expected_file_name = [file for file in context.kickstart_current_run_input_files if collection in file][0]
        console_printer.print_info(f"Expected File Name: {expected_file_name}")
        expected = file_helper.get_contents_of_file(expected_file_name, False).splitlines()[1:]
