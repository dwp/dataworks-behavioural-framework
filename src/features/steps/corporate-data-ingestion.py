...
def step_(context, type):
    if type == "source":
        ...
        console_printer.print_error_text(f"Executing: clean s3 '{type}' prefix. Unknown type.")
...
def step_(context, location_type):
    if location_type == "source":
        ...
        aws_helper.put_object_in_s3("foobar",
...
def step_(context, step_type):
    ...
    context.s3_destination_prefix = os.path.join(context.s3_destination_prefix, step_type)
...
def step_(context, key):
    response = list_objects_from_s3_with_retries(context.corporate_storage_s3_bucket_id,
                                                 context.s3_source_prefix)
    if len(response) == 0:
        AssertionError("Unable to retrieve file from S3")
    message = aws_helper.get_s3_object(None, context.corporate_storage_s3_bucket_id, response)
    ...
    aws_helper.put_object_in_s3(message_compressed,
                                context.corporate_storage_s3_bucket_id,
                                response)
