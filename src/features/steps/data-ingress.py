...
def step_impl(context):
    ...
    while time.time() < t1 and scaled == False:
        if aws_helper.instance_count_by_tag("aws:autoscaling:groupName", "data-ingress-ag") == 1:
            scaled = True
...
def step_impl(context):
    ...
    scaled = False
    while time.time() < t1 and scaled == False:
        if aws_helper.instance_count_by_tag("aws:autoscaling:groupName", "data-ingress-ag") == 0:
            scaled = True
            console_printer.print_info("instance count reached desired size within the time frame given")
        time.sleep(5)
    if scaled == False:
