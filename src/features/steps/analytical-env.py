...
def step_non_sc_role_assumed(context):
    ...
    arn_value = analytical_env_helper.generate_policy_arn(context.aws_acc, context.analytical_test_e2e_role)
...
def step_non_sc_role_assumed(context):
    team_databases = ["uc_ris_redacted", "uc_ris_unredacted", "ucs_opsmi_redacted", "ucs_opsmi_unredacted"]
...
@when("The user attempts to read data tagged pii=false from each of the team directories in the published S3 bucket")
...
def setup_test_file_in_s3(context, tag_map):
    ...
    file_helper.create_local_file(context.analytical_test_data_s3_location['file_name'], local_dir)
    ...
    file_helper.delete_local_file(context.analytical_test_data_s3_location['file_name'], local_dir)
